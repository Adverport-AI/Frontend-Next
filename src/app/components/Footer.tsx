import Link from "next/link";
import { ContactForm } from "./ContactForm";
import svgPaths from "../../imports/svg-2ykxxdmmup";

function LogoWhite() {
  return (
    <div className="h-[37.314px] w-[100px]">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 100 37.3135">
        <g id="logo white">
          <g id="Group 2">
            <path d={svgPaths.p3e243280} fill="white" />
            <path clipRule="evenodd" d={svgPaths.p60f7f80} fill="white" fillRule="evenodd" />
            <path d={svgPaths.p4f87cf0} fill="white" />
            <path clipRule="evenodd" d={svgPaths.p2fcaaa60} fill="white" fillRule="evenodd" />
            <path d={svgPaths.p2c68200} fill="white" />
            <path d={svgPaths.pd9c8200} fill="white" />
            <path clipRule="evenodd" d={svgPaths.p24fb9700} fill="white" fillRule="evenodd" />
            <path clipRule="evenodd" d={svgPaths.p3dbee080} fill="white" fillRule="evenodd" />
            <path d={svgPaths.pc074f00} fill="white" />
          </g>
          <path d={svgPaths.p383bd500} fill="white" />
        </g>
      </svg>
    </div>
  );
}

const socialLinks = [
  {
    label: "Instagram",
    href: "https://www.instagram.com/adverportcom/",
    icon: (
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
      </svg>
    ),
  },
  {
    label: "X",
    href: "https://x.com/adverportcom",
    icon: (
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    ),
  },
  {
    label: "Facebook",
    href: "https://www.facebook.com/adverportcomtr/",
    icon: (
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
      </svg>
    ),
  },
  {
    label: "LinkedIn",
    href: "https://www.linkedin.com/company/adverport/",
    icon: (
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
  },
];

const footerSections = [
  {
    title: "Ürün",
    links: [
      { label: "Özellikler", href: "/ozellikler" },
      { label: "Premium", href: "/premium" },
      { label: "Markalar", href: "/markalar" },
      { label: "Influencer", href: "/influencer" },
      { label: "Alışveriş Sever", href: "/alisveris-sever" },
    ],
  },
  {
    title: "Şirket",
    links: [
      { label: "Hakkımızda", href: "/hakkimizda" },
      { label: "Blog", href: "/blog" },
      { label: "İletişim", href: "/iletisim" },
    ],
  },
  {
    title: "Destek",
    links: [
      { label: "Sık Sorulan Sorular", href: "/sss" },
      { label: "Yardım Merkezi", href: "/yardim-merkezi" },
    ],
  },
  {
    title: "Yasal",
    links: [
      { label: "Ziyaretçi ve Kullanıcı Sözleşmesi", href: "/yasal/ziyaretci-ve-kullanici-sozlesmesi" },
      { label: "Üyelik Aydınlatma Metni", href: "/yasal/uyelik-aydinlatma-metni" },
      { label: "Websitesi Aydınlatma Metni", href: "/yasal/websitesi-aydinlatma-metni" },
      { label: "İletişim Aydınlatma Metni", href: "/yasal/iletisim-aydinlatma-metni" },
    ],
  },
];

interface FooterProps {
  showContact?: boolean;
}

export function Footer({ showContact = false }: FooterProps) {
  return (
    <footer id={showContact ? "contact" : undefined} className="bg-black border-t border-white/10 py-16">
      <div className="max-w-7xl mx-auto px-4">

        {showContact && (
          <div className="mb-16 max-w-2xl">
            <h2 className="text-2xl sm:text-3xl font-bold text-white mb-2">İletişime Geç</h2>
            <p className="text-white/60 text-sm mb-8">
              Sorularınız, iş birliği teklifleriniz veya destek talepleriniz için bize yazın.
            </p>
            <ContactForm />
          </div>
        )}

        <div className={`grid grid-cols-2 md:grid-cols-6 gap-10 md:gap-8 mb-12 ${showContact ? "border-t border-white/10 pt-12" : ""}`}>
          {/* Logo + Tagline + Social Icons */}
          <div className="col-span-2">
            <Link href="/" className="inline-block mb-4">
              <LogoWhite />
            </Link>
            <p className="text-white/60 text-sm leading-relaxed max-w-[240px]">
              Alışveriş yap ve paylaş, kazanç sağla
            </p>
            <div className="flex items-center gap-4 mt-5">
              {socialLinks.map((social) => (
                <a
                  key={social.label}
                  href={social.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-white/40 hover:text-white transition-colors"
                  title={social.label}
                >
                  {social.icon}
                </a>
              ))}
            </div>
          </div>

          {/* Footer Sections */}
          {footerSections.map((section) => (
            <div key={section.title}>
              <h3 className="text-white font-semibold mb-4 text-sm">{section.title}</h3>
              <ul className="space-y-3">
                {section.links.map((link) => (
                  <li key={link.label}>
                    <Link href={link.href} className="text-white/60 hover:text-white transition-colors text-sm">
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>
          ))}
        </div>

        <div className="border-t border-white/10 pt-8">
          <p className="text-white/40 text-sm text-center md:text-left">
            &copy; 2026 Adverport Teknoloji ve Yazılım Hizmetleri A.Ş. Tüm hakları saklıdır.
          </p>
        </div>

      </div>
    </footer>
  );
}
